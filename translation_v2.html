<script type="text/javascript">
	...
	var fbRegexStr = 'https?://(www.|m.|l.|web.|business.)?(d{4,5}.od.)?(business.)?(facebook|instagram|whatsapp).com(/.*)?';

	function isValidFbeRedirectUri(uri) { return new RegExp(fbRegexStr).test(uri) }

  var kyteWebAuthUrl = 'https://stage.web.kyteapp.com/external-login';
  ...
  function setButtonLink() {
    ...
    if (isValidFbeRedirectUri(redirectUri)) {
      ...
      if (!link) return false;
     	var href = kyteWebAuthUrl + "?redirect_uri=" + encodeURIComponent(redirectUri);
...
document.addEventListener("DOMContentLoaded", function () {
  var isoLang = getBrowserLang()
  var useLang = convertBrowserLang(isoLang.toLocaleLowerCase())
  var translationKeys = translations[useLang]
  for (var key in translationKeys) {
    document.getElementById(key).innerHTML = translationKeys[key]
  }
  var linkKeys = links[useLang]
